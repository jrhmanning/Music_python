...
T =388 
framework = 'IRMOF1'
parentdir = Path('~/scratch/01_music_IRMOF_solvent/experiments/{0}/{1}/'.format(species[-1], T))
#istm = ['''### your pressure points here###''']
##################
#Writes a bashscript for actually running your simulations, compatible with SLURM
def GcmcRunWriter(Species, T, framework, parentdir, dirout, isotherm, directory):
...
cp {4:02d}.postfile ../postfiles/

""".format(Species, T, dirout, parentdir, directory))
...
for directory in range(1, 17):
    GcmcRunWriter(species[-1], T, framework, parentdir, './{0:02d}/'.format(directory), None, directory)
