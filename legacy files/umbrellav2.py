...
IntParams = {
"Zinc":(2.763, 62.34, 0),
"Carbon":(3.898, 47.81, 0),
"Hydrogen":(3.195, 7.642, 0),
"Oxygen":(3.3, 48.11, 0),
"Oxygen_clust":(3.66, 0.069, 0),
"Oxygen_ligand":(3.3, 48.11, 0),
...
#            print(name[1][-1])
            ...
            if xt == 'pmap':
                if len(name) == 5:
                    filelist[name[3]] = [name[0], name[3][-1]]
                elif len(name) == 4:
                    filelist[name[1]] = [name[0], name[2][-1]]
                else:
            ...
            if xt == 'emap':
                if len(name) == 5:
                    ...
                    print("Warning, emap life name isn't framework_vX.atom_vX.emap!")
                    filelist[name[1]] = 0
#    print(molfilelist)
...
def AtmAtmMover(version, elements, version, dxout = "./mapgen/test/"):
    tot_el_list = []
    ...
    with open("{0}atom_atom_file" .format(dxout), "w") as file:
        ...
        for i in range (0, len(MOF_el_list)):
             ...
             for j in range (i, len(MOF_el_list)):
                 file.write("""{0} {1} LJ OFF\n""".format(MOF_el_list[i], MOF_el_list[j]))
             for k in elements:
                 file.write("""{0} {1} LJ SIG@LBMIX EPS@LBMIX HICUT@{2}\n""".format(MOF_el_list[i], elements[k], hicut))
            file.write("\n")
        for i in range (0, len(elements)):
            file.write("""{0} {0} LJ SIG@{1} EPS@{2} HICUT@{3}\n""".format(elements[i], IntParams[elements[i]][0], IntParams[elements[i]][1], hicut))
            for j in range (i, len(elements)):
                file.write("""{0} {1} LJ SIG@LBMIX EPS@LBMIX HICUT@{2}\n""".format(elements[i], elements[k], hicut))
            file.write("\n")
        ...
        for i in range (0, len(MOF_el_list)):
            ...
            for k in elements:
                file.write("""{0} {1} COUL OFF\n""".format(MOF_el_list[i], elements[k])
        for i in range (0, len(elements)):
...
def SorbSorbWriter(Species, elements, version, dxout = "./mapgen/", pmap = True):
    file = "sorb_sorb_file"
    with open("{0}{1}" .format(dxout, file), 'w') as file:
        file.write("{0} {0} NCOUL OFF\n{0} {0} COUL OFF\n\n" .format(framework)) #MOF-MOF interactions
        for i in Species:
            if pmap == True:
                file.write("{0}_v{2} {1} NCOUL MAP@{1} FAST ".format(i, framework, version))
                for j in elements:
                    file.write("{0}@PMAP@{2}.{0}_v{1}.pmap " .format(j, version, framework))
            else:
                file.write("{0}_v{2} {1} NCOUL BASIC LJ FAST".format(i, framework, version))
...
def Intsetup(mol_name, interactions_version, elements, dxout, pmap = True):
    AtmAtmMover(interactions_version, elements, dxout)
    SorbSorbWriter(mol_name, elements, interactions_version, dxout, pmap)
...
python ./isothermextractor.py""".format(Species, T, dirout))
...
IRMOF1.{0}_v{1}.res         # Restart File to write to                   
IRMOF1.{0}_v{1}.con          # Configuration File                   
------ Atomic Types --------------------------------------------------
...
./IRMOF1.{0}_v{1}.con                    # basename for config files
1, {3}                          # first and last file numbers
post.ctr.out                       # name for new ctrlfile that will regenerated
IRMOF1.{0}_v{1}.{2}K.post          # Base name for output files
...
sorb_el_list = []
for i in el_list:
    sorb_el_list.append("{0}_sorb" .format(i))
print(sorb_el_list)
for i in sorb_el_list:
    #print(i)
    directorymaker('./maps')
    if filecheck(i, interactions_version, 'pmaps', './maps', framework)[0] == True:
        ...
        if filecheck(i, interactions_version, 'mol', './molecules')[0] == True:
            if filecheck(species, interactions_version, 'mol', './molecules')[1] == True:
                ...
                print("{0} already exists as a molecule file but with the wrong version. I'll need to make a new molecule file for {0}" .format(species))
                molwrite("{0}_pmap" .format(i), [i], None, None, interactions_version, "./molecules/")
        else:
...
for directory in range (1,17):
    ...
    Intsetup(species, interactions_version, sorb_el_list, './experiments/{0}_v{1}/{2}/{3:02d}/'.format(species, interactions_version, T, directory))
    ...
    PressureFileWriter(species, T, satP, istm, './experiments/{0}_v{1}/{2}/{3:02d}/'.format(species, interactions_version, T, directory))
